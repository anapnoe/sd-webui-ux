...
def styles_insert_update_db(db_tables_pages):
...
def styles_init_refresh_db():
    logger.info("Initializing and refreshing styles database")
    styles_insert_update_db(db_tables_pages)
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as anapnoe_sd_uiux_db_styles:
        ...
        refresh_button_styles.click(fn=styles_init_refresh_db, inputs=[], outputs=[])

    return (anapnoe_sd_uiux_db_styles, 'Styles DB', 'anapnoe_sd_uiux_db_styles'),

#script_callbacks.on_app_started(lambda _, app: api_uiux_db(_, app, db_tables_pages)) we dont need to add to the api for now
